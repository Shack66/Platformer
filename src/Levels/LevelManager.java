...
public class LevelManager {
	...
	private void createWater() { //Para los srites del agua
	...
	public void loadNextLevel() {
		...
		game.getPlaying().setMaxLvlOffsest(newLevel.getLvlOffset());
	...
	public void draw(Graphics g, int lvlOffset) {	
		for (int j = 0; j < Game.TILES_IN_HEIGHT; j++)
			for (int i = 0; i < levels.get(lvlIndex).getLevelData()[0].length; i++) {
				...
				if (index == 48)
					 // Si el índice es 48, dibuja un sprite de agua animado
					g.drawImage(waterSprite[aniIndex], x, y, Game.TILES_SIZE, Game.TILES_SIZE,  null);
				else if (index == 49)
					// Si es 49, dibuja un sprite de agua estático
					g.drawImage(waterSprite[4], x, y, Game.TILES_SIZE, Game.TILES_SIZE,  null);
				else 
	                // Dibuja sprites del nivel para cualquier otro índice
